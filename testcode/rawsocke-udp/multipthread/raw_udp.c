...
void udp_packet_buffer_generate(test_buff_t *pkt_buffer)
{
	char * buffer;
	memset(pkt_buffer, 0,sizeof(test_buff_t));
    buffer = pkt_buffer->buffer;
	...
	//udp header
	udp->udph_srcport = htons(2000);
	udp->udph_destport = htons(4000);
	...
	ip->iph_sourceip = inet_addr("192.168.1.100");
	// The destination IP address
	ip->iph_destip = inet_addr("192.168.1.1");
	/******************end fill ip header**********************/
	/********************fill eth header*********************/
	...
	pkt_buffer->len = sizeof(struct ether)+sizeof(struct ipheader)+sizeof(struct udpheader)+sizeof(data);
}

void *send_apcket_loop(void *_param)
{
	test_buff_t *pkt_buffer = (test_buff_t *) _param;
	#if 0
	struct ether *eth = (struct ether *)malloc(sizeof(struct ether));
	...
	{
		printf("create SOCK_RAW socket OK. sd=%d\n",sd);
	}
 #else
	int sd;
	char ifname[16]= "eth0";
    ...
    device.sll_pkttype   = PACKET_HOST;
    device.sll_halen     = 6;
    
    if(bind(sockfd, (struct sockaddr*)&device, addrlen) < 0)
    {
	...
	{
		//send udp packet
		if(sendto(sd, (void *)pkt_buffer->buffer, pkt_buffer->len, 0, (struct sockaddr *)&device, sizeof(device)) < 0)
...
int main(int argc, char *argv[])
{
	int i;
	int rv;
	test_buff_t pkt_buffer;
	udp_packet_buffer_generate(&pkt_buffer);

	for(i = 0; i < THREAD_NUM; i++)
	{
		rv = pthread_create(&tid[i], NULL,  send_apcket_loop, (void *)&pkt_buffer);
		if(rv)
		{
			printf("Failed Create Thread for send_apcket_loop\n");
		}
		else
		{
			pthread_detach(tid[i]);
			printf("Success Create Thread-%ld for send_apcket_loop\n",tid[i]);
		}
	}
