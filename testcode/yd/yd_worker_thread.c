...
static void *worker_func(void *arg) {
    ...
    printf("cpu nums = %d,  current thread id = %ld, bind cpu core id = %d \n", numCPU, thread_id, core_id);
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    //hdr.len = pkthdr->caplen; 
    
    //下面的这个函数就是把传输过来的数据进行一次重装放进hdr里面，然后再其他线程里面进行进一步的http的数据解析
    //pfring_parse_pkt( me->thread_id, (u_char *) packet, &hdr, 5, 1, 1);
    //int i = http_cap_filter_main( me->thread_id , (u_char*)packet, &hdr, ctx);
    //if(i !=0) return;
