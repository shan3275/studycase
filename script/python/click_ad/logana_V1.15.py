...
VERSION="LOGANA V1.15, 2018/2/05; copyright by less (less@beescast.com)"
...
def  dump_src_topx(utopx, rtopx, ofile):
    ...
    UITEMS = sorted(DU_HOST_TAB.items(), key=lambda TU: TU[1], reverse=True) 
    print "\n ----------------------------------------------------------------------------------------"
    print >> ofile, "\n ----------------------------------------------------------------------------------------"
    print " %s:" % ("Source")
            ...
            for kdict in kdicts:
                if kdict.has_key('di') and uri_compare(kdict['di'], DUF) == True:
                    if RUF.strip() != '' and uri_compare(kdict['di'].split('\.')[1],RUF) != True:
                        DUF_HOST=DUF.split('/',1)[0]
                        RUF_HOST_LIST=RUF.split('/')
                        if len(RUF_HOST_LIST) >= 3:
                            RUF_HOST=RUF_HOST_LIST[2]
                            KHIT = 1
                            break
...
def main():
    ...
    if options.dutopx > 0:
        ...
        dump_tab_topx("Dest host",   DU_HOST_TAB, options.dutopx, ofile)
    ...
    if options.rutopx > 0:
        ...
        dump_tab_topx("Refer HOST",  RU_HOST_TAB, options.rutopx, ofile)
